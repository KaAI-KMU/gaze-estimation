...
from model import GazeEstimationModel_vgg16, GazeEstiamationModel_resent18, GazeEstimationPreactResnet
from dataloader import EstimationHdf5DatasetMyDataset
...
if __name__ == "__main__":
    ...
    root_parser.add_argument('--model_loc', type=str, required=False, help='path to the model files to evaluate', action="append", default='../checkpoints/fold_0/')
    root_parser.add_argument('--hdf5_file', type=str, default="custum/KaAI_dataset.hdf5")
    root_parser.add_argument('--num_io_workers', default=8, type=int)
    root_parser.add_argument('--loss_fn', choices=["mse"], default="mse")
    root_parser.add_argument('--model_base', choices=["vgg16", "resnet18", "preactresnet"], default="vgg16")
    ...
    _models = {
        "vgg16": partial(GazeEstimationModel_vgg16, num_out=_param_num.get(hyperparams.loss_fn)),
        "resnet18": partial(GazeEstiamationModel_resent18, num_out=_param_num.get(hyperparams.loss_fn)),
        "preactresnet": partial(GazeEstimationPreactResnet, num_out=_param_num.get(hyperparams.loss_fn))
    }

    test_subjects = [[0,1,2,3]]
    ...
    ensemble = sum([os.path.isfile(os.path.join(hyperparams.model_loc,s)) for s in os.listdir(hyperparams.model_loc)]) == len(os.listdir(hyperparams.model_loc))

    if ensemble:
        _models_list = []
        for model_file in tqdm(os.listdir(hyperparams.model_loc), desc="Ensemble Evaluation; Loading models..."):
            _model = _models.get(hyperparams.model_base)()
            _model.load_state_dict(torch.load(os.path.join(hyperparams.model_loc, model_file)))
            _model.to("cuda:0")
            _model.eval()
            _models_list.append(_model)

        for fold_idx, test_subject in enumerate(test_subjects):
            data_test = EstimationHdf5DatasetMyDataset(h5_file=h5py.File(hyperparams.hdf5_file, mode="r"), subject_list=test_subject)
