...
class Camera_Model(abc.ABC):
    cam_type =None  # overwrite in subclasses, used for saving/loading
    ...
    @abc.abstractmethod
    def undistort(self, img: np.ndarray) -> np.ndarray: #cw
        ...

    @abc.abstractmethod
    def unprojectPoints(
        self, pts_2d: np.ndarray, use_distortion: bool = True, normalize: bool = False #cw
    ) -> np.ndarray:
        ...
    ...
    def projectPoints(
        self,
        object_points,
        rvec: T.Optional[np.ndarray] = None, #cw
        tvec: T.Optional[np.ndarray] = None, #cw
    ...
    def from_file(
        ...
        are available, we use intrinsics.
